...
//	Sequence *seq = Sequence::create(DelayTime::create(0.1f), CallFunc::create(boost::bind(&SneakyButtonSkinnedBase::setPos, &*attack, player, 200, 300)), NULL);
//	attack->runAction(RepeatForever::create(seq));
...
CToolbar * CToolbarManager::createFrostToolbar(cocos2d::Size const &visibleSize, CPlayer *player, CMainScene *scene)
{

	auto m_toolbar = CToolbar::create();

	auto button1 = m_toolbar->createButton(Point(visibleSize.width * 0.8f, visibleSize.height * 0.13f), Rect(0, 0, 64.0f, 64.0f), "frost_1");
	button1->getButton()->m_onClick.connect(boost::bind(&CMainScene::frostCircle, &*scene));
